...
import pandas as pd
import numpy as np
from scriptRandomForest2 import clean_titanic
...
dftrain=dataTot.head(891)
dftest=dataTot.tail(418)
...
dftestnorm=dataTot.tail(418)
dftrain['Ligne'] = list(range(0,len(dftrain)))
dftrain = dftrain.set_index('Ligne')
dftrain ['Survived'] = trainConst['Survived'].astype(int)
dftrainnorm['Ligne'] = list(range(0,len(dftrainnorm)))
dftrainnorm = dftrainnorm.set_index('Ligne')
dftrainnorm ['Survived'] = trainConst['Survived'].astype(int) 
...
dfsvmresult.to_csv('titanicSVM.csv', index=False) #Change pathname
